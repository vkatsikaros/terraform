	...
	case addrs.ManagedResourceMode:
		...
		if !exists {
			diags = diags.Append(&hcl.Diagnostic{
				...
				Detail:   fmt.Sprintf("The provider %s does not support resource type %q.", cfg.ProviderConfigKey(), cfg.Type),
	...
	case addrs.DataResourceMode:
		...
		if !exists {
			diags = diags.Append(&hcl.Diagnostic{
				...
				Detail:   fmt.Sprintf("The provider %s does not support data source %q.", cfg.ProviderConfigKey(), cfg.Type),
				Subject:  &cfg.TypeRange,
			})
			return nil, diags.Err()
		}

...
func (n *NodeApplyableModuleVariable) ReferenceOutside() (referenceablePath, referencerPath addrs.ModuleInstance) {
	// Module input variables are referenced from their own module (using
	// var.foo-type references) but they depend on values from the _parent_
	// module, depending on what is referenced in the "module" block in the
	// module call.
	//
	// If we're in the root module then we can't actually reference anything,
	// so we'll just use the root module as a placeholder. This behavior is
	// automatic in path.Parent when called on the root module.
	path := n.Addr.Module
